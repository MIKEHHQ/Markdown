...
{
	"sum", /* The command string to type. */
	"\r\nsum:\r\n sum of numbers  \r\n",
	prvsumCommand, /* The function to run. */
	-1 /* No limit of parameters num. */
};
static BaseType_t prvmulCommand( char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString );
static const CLI_Command_Definition_t xmul =
{
	"mul", /* The command string to type. */
	"\r\nmul:\r\n mul of numbers  \r\n",
	prvmulCommand, /* The function to run. */
	-1 /* No limit of parameters num. */
};
...
void vRegisterSampleCLICommands( void )
{
	...
	FreeRTOS_CLIRegisterCommand( &xmul );
...
static UBaseType_t uxParameterNumber = 1;
	...
	if(strcmp(pcParameter,"read")==0)
	{
		memset( pcWriteBuffer, 0x00, xWriteBufferLen );
		if(led_read())
		{
			sprintf( pcWriteBuffer, "led is on.");
		}
		else sprintf( pcWriteBuffer, "led is off.");
	}
